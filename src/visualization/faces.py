"""
Visualizations for the face factor demo.

The 3D PCA view and the persistence diagram are identical to the MNIST demo and
are reused from `visualization.mnist`; only the frames that show images differ,
because the faces are rendered at their own resolution and their factor is not
an angle (a smile has no degrees).
"""
...
def render_samples_frame(fig, X, captions, title, pixel_mean=None,
    ...
    """
    A grid of dataset frames spread evenly over the whole dataset, so every
    component gets a share of the rows. `captions` holds one already formatted
    factor value per observation (units differ per component).
    """
    n_show = 20
...
def render_spline_frame(fig, state, pca_data, colors, exp, pca_basis, spline,
                        ...
                        image_shape=(64, 64)):
    """
    The face reconstructed at the current spline coordinate next to the PCA
    view. `state["factor_label"]` names the factor of the selected component,
